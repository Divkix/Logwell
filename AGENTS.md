...
- `vite.config.ts` (vp: staged formatting, lint ignores `sdks/**`), `knip.json`, `pnpm-workspace.yaml` (pnpm settings + `allowBuilds`), `.vite-hooks/pre-commit` (`vp staged`, `vp check`, knip, conditional SDK check)
