...
- **`dependabot.yml`**: weekly updates across 6 ecosystems (grouped minors/patches, svelte + testing groups, majors separate), prefix `deps`. Toolchain (`vite-plus`, `vite`, `vitest`, `@vitest/*`, `@voidzero-dev/vite-plus-core`) is `ignore`d in bun + sdk-ts entries — bump via `vp migrate` only, never solo.
